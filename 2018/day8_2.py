...
def test_day8_1():
    ...
    assert result == 66, "Result: " + str(result)
...
def metadata_sum(node):
    ...
    metadata = [m - 1 for m in node.metadata if 1 <= m <= len(node.children)]
    return sum([metadata_sum(node.children[i]) for i in metadata])
