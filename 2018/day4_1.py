...
def day4_1(input):
    ...
    guards_sleep_sum = {id: sum(sleep) for id, sleep in guards_sleep.items()}
    guard_id_max_sleep = max(
        guards_sleep_sum, key=lambda id: guards_sleep_sum[id])
    return int(guard_id_max_sleep) * max(
        range(60), key=lambda min: guards_sleep[guard_id_max_sleep][min])
